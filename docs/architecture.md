...
Harness is a middle layer for coding-agent workflows. Skills and prompts can
describe a process, but Harness makes the process executable: the FSM owns the
...
Codex writes code, runs tools, and produces natural-language reasoning. Harness
does not ask Codex to remember the workflow. Instead, Harness exposes only the
...
concrete coordinates and structural edge routes. Harness supplies semantic model
...
The model submits structured payloads through Harness-controlled tools. Harness
...
This is the core boundary: Codex performs the work; Harness constrains the
process around that work.

## Artifacts And Runs

Each invocation creates a fresh run directory under `.harness/runs/<runId>/`.
...
their `fsms/` and bundled `skills/`; Harness owns the generated executable and
