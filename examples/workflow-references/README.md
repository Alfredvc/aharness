...
These docs were previously in `docs/` as if they were framework specs. They are
not. The framework ships mechanisms (`aharness_submit` dynamic-tool dispatch,
Codex `request_user_input` server-request handling for model-originated owner
prompts, per-run inspection snapshots, final artifacts, hooks, verifier) and is
unopinionated about which workflow you build on top.
