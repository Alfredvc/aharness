 ...
 * harness runtime's UDS endpoint.
 *
 ...
 * the same Node interpreter that runs the harness CLI.
 */
const HEADER = '#!/usr/bin/env bash\nset -euo pipefail\n';

export interface RenderHookKindScriptInput {
  /** Absolute path to the harness runtime's hook UDS. */
...
/**
 * pre_tool_use.sh body — forwards a PreToolUse hook payload to the harness UDS
...
/**
 * post_tool_use.sh body — forwards a PostToolUse hook payload to the harness
 ...
 * the harness UDS with the `USER_PROMPT_SUBMIT` framing tag.
 */
...
function renderKindScript(
  ...
  return (
    HEADER +
    `# ${scriptFilename}: forward codex stdin to the harness UDS as a ${tag} frame.\n` +
