 ...
 *   - the runtime/verifier to build submit-tool definitions used by
...
import type { AharnessMeta, SchemaSidecar, SubmitToolDef } from './types.js';
...
/**
 * Tool defs derived from the machine's submit-exit catalogue.
 *
...
/**
 * Build submit-tool defs for every accepted submit exit declared by `machine`.
 *
    ...
    for (const exitName of Object.keys(meta.exits)) {
      ...
      if (exit.kind === 'await') {
        throw new BuildExitToolsError(
          ...
          stateId,
          exitName,
          `buildExitTools: await exit '${stateId}::${exitName}' is retired and unreachable from accepted metadata.`,
        );
      }
