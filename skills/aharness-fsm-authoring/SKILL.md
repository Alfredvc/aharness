...
- Use object-form `clearOnEntry` with any non-empty combination of `cwd`, `model`, and `reasoningEffort`, for example `{ model: 'gpt-5.1-codex' }`, `{ reasoningEffort: 'high' }`, `{ model: 'gpt-5.1-codex', reasoningEffort: 'high' }`, or `{ cwd: '/absolute/path', model: 'gpt-5.1-codex', reasoningEffort: 'high' }`.
...
- Use only `none`, `minimal`, `low`, `medium`, `high`, or `xhigh` for `reasoningEffort`. Effort may omit `model`; aharness resolves the target model from Codex `config/read({ cwd })`, then the `model/list` default/first fallback. Static declarations are checked by `aharness verify`; dynamic `cwd` may defer effort support checks to runtime preflight. Dynamic `model` and `reasoningEffort` callbacks are not supported.
